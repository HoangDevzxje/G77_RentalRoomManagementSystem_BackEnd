...
function decryptIdentityForView(contract) {
  ...
  if (!iv || iv.status !== "verified" || !iv.ocrData?.cccdEncrypted) {
    return;
  }
...
exports.cloneContract = async (req, res) => {
  try {
    ...
    const existingClone = await Contract.findOne({
      clonedFrom: old._id,
      status: { $in: ["draft", "pending", "active", "signed"] },
    }).lean();
...
exports.approveExtension = async (req, res) => {
  try {
    ...
    const conflict = await Contract.findOne({
      ...
      roomId: contract.roomId,
      isDeleted: false,
      status: { $nin: ["voided", "terminated"] },
      "contract.startDate": { $lte: newEnd },
      "contract.endDate": { $gte: oldEndDate },
