...
const addRoommate = async (req, res) => {
  ...
  if (userIds.includes(requesterId)) {
    return res
      .status(400)
      .json({ message: "Không thể thêm chính mình vào danh sách" });
  }
...
const removeRoommate = async (req, res) => {
  ...
  if (userIds.includes(requesterId)) {
    return res
      .status(400)
      .json({ message: "Không thể tự xóa chính mình khỏi phòng" });
  }
...
const getMyRoommates = async (req, res) => {
  ...
  try {
    ...
    const room = await Room.findById(roomId).populate({
      ...
      select:
        "-password -passwordResetToken -passwordResetExpires -accessToken -refreshToken",
      populate: {
        path: "userInfo",
    ...
    res.json({
      ...
      data: {
        ...
        currentCount: room.currentTenantIds.length,
        canAddMore:
          isMainTenant && room.currentTenantIds.length < room.maxTenants,
        isMainTenant,
        roommates,
      },
    });
...
const getRoommateDetail = async (req, res) => {
  ...
  try {
    ...
    const account = await Account.findById(userId)
      .select(
        "-password -passwordResetToken -passwordResetExpires -accessToken -refreshToken -__v"
      )
      .populate({
        path: "userInfo",
...
const leaveRoommate = async (req, res) => {
  ...
  try {
    ...
    if (!room) {
      return res.status(403).json({
        message: "Phòng không tồn tại hoặc bạn không thuộc phòng này",
      });
    }

    const mainTenantId = room.currentTenantIds?.[0]?.toString();

    // Chỉ roommate được rời phòng bằng API này
    if (mainTenantId && mainTenantId === requesterId) {
      return res.status(403).json({
        message:
          "Bạn là người đứng tên hợp đồng nên không thể rời phòng bằng chức năng này",
      });
    }

    // Remove chính mình khỏi currentTenantIds
    room.currentTenantIds = room.currentTenantIds.filter(
    ...
    // Notification cho main tenant (nếu còn)
    const requester = await Account.findById(requesterId)
      .populate("userInfo", "fullName")
      .lean();
    ...
    if (mainTenantId) {
      const notification = await Notification.create({
        landlordId: room.buildingId?.landlordId,
        createByRole: "system",
        title: "Roommate đã rời phòng",
        content: `${
          requester?.userInfo?.fullName || "Một roommate"
        } đã rời khỏi phòng ${room.roomNumber}`,
        target: { residents: [mainTenantId] },
      });
      ...
      if (io) {
        ...
        io.to(`user:${mainTenantId}`).emit("new_notification", payload);
        io.to(`user:${mainTenantId}`).emit("unread_count_increment", {
          increment: 1,
        });
      }
    }
    ...
    await session.abortTransaction();
    console.error("Leave roommate error:", error);
    return res.status(500).json({ message: "Lỗi hệ thống, vui lòng thử lại" });
  } finally {
    session.endSession();
  }
